function output = findmddgasonly (data) % Add gas/elec consumption fields, discount fields, date current/all fields, Company List fields, later on
            ...
            for e=1:1:length(rname)
                ...
                for d=1:1:length(arch)
                    ...
                    if conval.(arch{d}).Gas==0
                        E1TAB=((VAT).*( (GasU*GasCon) )) -(DDF/2);% no standing gas charge for non gas customers
                        E7TAB=(VAT).*( (GasSt.*365)+(GasU*GasCon) );
                    else
                        E1TAB=((VAT).*( (GasU*GasCon) )) -(DDF/2);
                        E7TAB=(VAT).*( (GasSt.*365)+(GasU*GasCon) );
                    end
