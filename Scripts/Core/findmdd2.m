function output = findmdd2 (data) % Add gas/elec consumption fields, discount fields, date current/all fields, Company List fields, later on
            ...
            for e=1:1:length(rname)
                ...
                for d=1:1:length(arch)
                    ...
                    if conval.(arch{d}).Gas==0
                        E1TAB=((VAT).*( (ElecSt.*365)+(Elec0.*ElecCon)+(GasU*GasCon) )) -DDF;% no standing gas charge for non gas customers
                        E7TAB=(VAT).*( (ElecSt.*365)+(Elec1.*ElecCon.*0.55)+(ElecN.*ElecCon.*0.45)+(GasSt.*365)+(GasU*GasCon) );
                    else
                        E1TAB=((VAT).*( (ElecSt.*365)+(Elec0.*ElecCon)+(GasSt.*365)+(GasU*GasCon) )) -DDF;
                        E7TAB=(VAT).*( (ElecSt.*365)+(Elec1.*ElecCon.*0.55)+(ElecN.*ElecCon.*0.45)+(GasSt.*365)+(GasU*GasCon) );
                    end
